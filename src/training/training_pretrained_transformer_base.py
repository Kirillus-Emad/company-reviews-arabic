...
def main():
    ...
    saved_config = os.path.join(TRANS_BASE_MODELS_DIR, 'config.json')
    resume = False
    if os.path.isfile(saved_config):
        import json as _json
        with open(saved_config) as _f:
            _saved_name = _json.load(_f).get('_name_or_path', '')
        resume = TRANS_BASE_MODEL_NAME in _saved_name or _saved_name in TRANS_BASE_MODEL_NAME

    model_source = TRANS_BASE_MODELS_DIR if resume else TRANS_BASE_MODEL_NAME
    num_epochs   = TRANS_BASE_RESUME_EPOCHS if resume else TRANS_EPOCHS
